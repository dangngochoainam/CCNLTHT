...
class PostsViewSet(viewsets.ViewSet, generics.ListAPIView):
    ...
    def add_post(self, request):
        ...
        if content:
            p = Posts.objects.create(title=title, content=content, user=user, image=image)
            if tag:
                t = Hastag.objects.create(name=tag)
                p.hagtags.add(t)
            if hagtags:
                hagtags = list(hagtags.split(","))
                for tag in hagtags:
                    t = Hastag.objects.get(name=tag)
                    p.hagtags.add(t)
            return Response(PostsDetailSerializer(p, context={'request': request.data}).data,
...
class PostsDetailViewSet(viewsets.ViewSet, generics.RetrieveAPIView,
    ...
    def add_comment(self, request, pk):
        content = request.data.get('content')
        if content:
            notif = Notification.objects.create(content='Comment', creator=request.user, posts=self.get_object(), user=self.get_object().user)
    ...
    def like(self, request, pk):
        ...
        if l.active == True:
            ...
            notif, _ = Notification.objects.get_or_create(content='Like', user=self.get_object().user, posts=self.get_object(), creator=user)
            # notif= Notification.objects.create(content='Like', user=self.get_object().user, posts=self.get_object(), creator=user)
        ...
        return Response(data=LikeSerializer(l).data, status=status.HTTP_200_OK)
    ...
    def get_like(self, request, pk):
        ...
        posts = self.get_object()
        counter_like = Like.objects.filter(posts_id=posts.id).count()

        print(counter_like)

        return Response(data=counter_like, status=status.HTTP_200_OK)
...
class UserViewSet(viewsets.ViewSet, generics.ListAPIView, generics.CreateAPIView, generics.RetrieveAPIView):
    ...
    @action(methods=['get'], detail=True, url_path='get-notifications')
    def get_notifications(self, request, pk):

        notifs = Notification.objects.filter(user=pk)
    ...
    def get_posts(self, request, pk):
        ...
        return Response(PostsDetailSerializer(posts, many=True).data,
                        status=status.HTTP_200_OK)
